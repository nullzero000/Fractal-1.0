...
const App = () => {
  ...
  const handleAnalyze = () => {
      if (inputText.length > 0) {
          ...
          setActiveLevel(0);
          setTimeout(() => { setIsLoading(false); setShowResultsContent(true); }, 2500); 
      }
  };
  ...
  return (
    <div className="supramente-terminal">
        ...
        {inputText ? (
          <div className="live-text">
            {inputText.split('').map((char, index) => {
               const color = getActiveColor(char);
               return (
                   <span key={index} style={{ color: color, textShadow: getTextShadow(color), transition: 'color 0.3s' }}>
                     {char}
                   </span>
               );
            })}
      ...
      {/* 3. NUEVO SYSTEM NAVBAR (UBICACIÓN ESTRATÉGICA) */}
      <nav className="system-navbar">
          
          {/* LURIAN (ARI) - FUEGO */}
          <button 
            className={`sys-btn ${colorSystem === 'ari' ? 'active' : ''}`} 
            onClick={() => setColorSystem('ari')}
          >
            <svg className="sys-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* ORTHODOX (INK) - PLUMA */}
          <button 
            className={`sys-btn ${colorSystem === 'ort' ? 'active' : ''}`} 
            onClick={() => setColorSystem('ort')}
          >
            <svg className="sys-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* AKASHIC (RGB) - OJO/RED */}
          <button 
            className={`sys-btn ${colorSystem === 'akashic' ? 'active' : ''}`} 
            onClick={() => setColorSystem('akashic')}
          >
            <svg className="sys-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {/* HERMETIC (GD) - ESTRELLA */}
          <button 
            className={`sys-btn ${colorSystem === 'gd' ? 'active' : ''}`} 
            onClick={() => setColorSystem('gd')}
          >
            <svg className="sys-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
      ...
      <footer className="terminal-footer">
        <p>EJE 13 / MODULAR SYSTEM v4.2</p>
      </footer>
    </div>
  );
};
